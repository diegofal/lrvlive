...
<form name="resellers" method="post" action="edit_ticket.php?tour_id={$smarty.get.tour_id}&code={$smarty.get.code}">
...
<option value="{$resellers[i].reseller_id}" {if $resellers[i].reseller_id==$smarty.get.reseller_id}selected="selected"{/if}>{$resellers[i].reseller_name}</option>
{/section}
</select><br><br>
<a href="javascript:document.resellers.submit();" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image32','','../WEB-INF/assets/images/booking/button-proceed-on.gif',1)"><img src="../WEB-INF/assets/images/booking/button-proceed-off.gif" name="Image32" width="107" height="32" border="0"></a></div>
</form>
</body>
</head>