...
<form name="editor_form" action="{$smarty.server.PHP_SELF}?subpage_id={$smarty.get.subpage_id}" method="post">
  ...
  <tr>
	  ...
	  <tr>
		<td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="outline-orange">
		  <tr>
			...
			<div align="left" class="txt-tahoma"><strong>Main content</strong></div>
			{else}
			<div align="left" class="txt-tahoma"><strong><a href="{$smarty.server.PHP_SELF}" class="submenu">Main content</a></strong></div>
			{/if}
			</td>
		  </tr>
	...
	{if !empty($smarty.get.subpage_id) || ($smarty.get.option==add)}
	  ...
	  <tr>
		<td valign="top" class="txt-tahoma"><strong>Tour block 1</strong></td>
		<td>
				<select name="page_link1">
					{section name=i loop=$tours}
					{assign var="page_link1_value" value="`$tours[i].tour_id`"}
					<option value="{$tours[i].tour_id}"{if $content.page_link1 == $page_link1_value} selected{/if}>{$tours[i].tour_home_name1}</option>
					{/section}                    
				</select>
		</td>
	  </tr>
	  <tr>
		<td valign="top" class="txt-tahoma"><strong>Tour block 2</strong></td>
		<td>
				<select name="page_link2">
					{section name=i loop=$tours}
					{assign var="page_link2_value" value="`$tours[i].tour_id`"}
					<option value="{$tours[i].tour_id}"{if $content.page_link2 == $page_link2_value} selected{/if}>{$tours[i].tour_home_name1}</option>
					{/section}                    
				</select>
		</td>
	  </tr>
	  <tr>
		<td valign="top" class="txt-tahoma"><strong>Tour block 3</strong></td>
		<td>
				<select name="page_link3">
					{section name=i loop=$tours}
					{assign var="page_link3_value" value="`$tours[i].tour_id`"}
					<option value="{$tours[i].tour_id}"{if $content.page_link3 == $page_link3_value} selected{/if}>{$tours[i].tour_home_name1}</option>
					{/section}                    
				</select>
		</td>
	  </tr>
	   <tr>
		<td valign="top" class="txt-tahoma"><strong>Tour block 4</strong></td>
		<td>
				<select name="page_link4">
					{section name=i loop=$tours}
					{assign var="page_link4_value" value="`$tours[i].tour_id`"}
					<option value="{$tours[i].tour_id}"{if $content.page_link4 == $page_link4_value} selected{/if}>{$tours[i].tour_home_name1}</option>
					{/section}                    
				</select>
		</td>
	  </tr>
