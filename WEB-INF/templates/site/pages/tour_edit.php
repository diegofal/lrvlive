...
//		print_r($_FILES);
		...
		if (!isset($error)) {
			...
			if (!empty($_FILES['tour_home_image']['name'])) $db->insert_file($db->tour, "tour_id", $tour_id, "tour_home_image", $_FILES, $_SERVER['DOCUMENT_ROOT']."/img/tours/", $large_dim="600", $thumb_dim="180", $both="1", $file_type="", $name="tour_home_image");
			if (!empty($_FILES['tour_big_image']['name'])) $db->insert_file($db->tour, "tour_id", $tour_id, "tour_big_image", $_FILES, $_SERVER['DOCUMENT_ROOT']."/img/tours/", $large_dim="600", $thumb_dim="386", $both="1", $file_type="", $name="tour_big_image");
		...
		if (!isset($error)) {
			...
			*/
			...
			if (!empty($_FILES['tour_home_image']['name'])) {
				$db->insert_file($db->tour, "tour_id", $tour_id, "tour_home_image", $_FILES, $_SERVER['DOCUMENT_ROOT']."/img/tours/", $large_dim="600", $thumb_dim="180", $both="1", $file_type="", $name="tour_home_image");
			}
			if (!empty($_FILES['tour_big_image']['name'])) $db->insert_file($db->tour, "tour_id", $tour_id, "tour_big_image", $_FILES, $_SERVER['DOCUMENT_ROOT']."/img/tours/", $large_dim="600", $thumb_dim="386", $both="1", $file_type="", $name="tour_big_image");				
			if (!empty($_FILES['tour_image1']['name'])) $db->insert_file($db->tour, "tour_id", $tour_id, "tour_image1", $_FILES, $_SERVER['DOCUMENT_ROOT']."/img/tours/", $large_dim="600", $thumb_dim="119", $both="1", $file_type="", $name="tour_image1");
			if (!empty($_FILES['tour_image2']['name'])) $db->insert_file($db->tour, "tour_id", $tour_id, "tour_image2", $_FILES, $_SERVER['DOCUMENT_ROOT']."/img/tours/", $large_dim="600", $thumb_dim="119", $both="1", $file_type="", $name="tour_image2");
