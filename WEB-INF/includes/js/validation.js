...
language["num"]		= " and must contain numbers";
language["email"]	= " must contain a valid e-mail address";
language["site"]	= " must contain a valid website address";
language["passwd"]	= " doesn't match";
language["invalidchar"]	= " contains invalid characters";
// -----------------------------------------------------------------------------
// define - Call this function in the beginning of the page. I.e. onLoad.
// n = name of the input field (Required)
// type= string, num, email, site, password (Required)
// min = the value must have at least [min] characters (Optional)
// max = the value must have maximum [max] characters (Optional)
// d = (Optional)

//!!!!!!!!!! dupa fieldul password trebuie sa urmeze fieldul retype care trebuie definit ca atare

//EX:
//	function init() {
/*	
	define('name', 'string', 'Contact Name');
...
---	definire parolei---
	define('passwd', 'password', 'Password',3,8);
	define('retype', 'retype', 'Retype Pass');
...
if ((type == "site") && (x.value == "")) { x.value = "http://";}
}
...
if (validateObject.type == "password") {
	var retypePsw = checkObjects[i+1].form.value;
	var invalid = " "; // Invalid character is a space
	if (validateObject.val != retypePsw) { errors += language['start'] + " " +validateObject.HTMLname + language['passwd'] + "\n"; }
		else 
			...
			if (validateObject.val.indexOf(" ") != -1) 
					{ errors += language['start'] + " " +validateObject.HTMLname + language['invalidchar'] + "\n"; }
...
if ((validateObject.val.indexOf("@") == -1) || (validateObject.val.charAt(0) == ".") || (validateObject.val.charAt(0) == "@") || (validateObject.len < 6) || (validateObject.val.indexOf(".") == -1) || (validateObject.val.charAt(validateObject.val.indexOf("@")+1) == ".") || (validateObject.val.charAt(validateObject.val.indexOf("@")-1) == ".")) { errors += language['start'] + language['field'] + validateObject.HTMLname + language['email'] + "\n"; }
   	...
   	if(validateObject.type == "site") {
	// Checking existense of "http://" and ".". 
	//  the "." required 
	// not directly precede or follow the "." or " "
	if ( (validateObject.val.indexOf("http://") == -1) || (validateObject.val.indexOf(".") == -1) || (validateObject.val.charAt(7) == ".") || (validateObject.val.charAt(validateObject.val.indexOf(".")+1) == "") || (validateObject.val.charAt(validateObject.val.indexOf(".")+1) == ".") )
		{ errors += language['start'] + language['field'] + validateObject.HTMLname + language['site'] + "\n"; }
	 }
