...
$current_reseller_id = $_SESSION['current_reseller_id']; //49
		...
		if ($departure_id != 0 && $order_id !=0 && $db->exist_value($db->order,'order_id',$order_id)) {
			...
			if ($_seats == 0) {
				...
				if ($sum <= $departure['boat_passengers']) {
					...
					$result = edit_order($order_fields, "order_id", $order_id, "bale booking 169");
...
$timeTmp = $dateTmp->format('H:i:s');
		...
		foreach($departures as $key => $departure){
		  ...
		  $query = "SELECT o.*, d.departure_tour_id AS order_tour_id FROM $db->order AS o
			...
			if ($departure['boat_passengers'] - $total_blocked != 0) {					
...
//		print_r($departures);
		...
		$smarty->assign("cur_date",$curent_date);

// Added by Carlos
		$dec_date =strtotime($cur_day_format. ' -1 days');
		$prev_date  = date('Y-m-d', $dec_date);

		$inc_date =strtotime($curent_date. ' +1 days');
		$next_date = date('Y-m-d', $inc_date);
...
//		$smarty->assign("unixtime",time());
		...
		$smarty->assign("page","bookings");
		$smarty->display('cms_resellers.tpl');	
		
if($_POST['command'] == "add")
{
	    
		$departure_id 		= (int)$_POST['dep_id'];
		$adult_price       	= $_POST['adult_price'];
		$child_price       	= $_POST['child_price'];
		$cname             	= $_POST['name'];
		$phone             	= $_POST['mobile'];
		$email             	= $_POST['email'];
		$name_arr          	= explode(" ",$cname);
		$_SESSION['Name'] 	= $cname; 
		$_SESSION['Phone'] 	= $phone; 
		$_SESSION['Email'] 	= $email; 
		$_SESSION['Adult'] 	= trim($_POST['adult']);
		$_SESSION['Kids'] 	= trim($_POST['kid']);	
		$tour_id			= trim($_POST['tour_id']);	
		
		$q  = array();
		$q[$departure_id] = array(
			"Adult" =>$_POST['adult'],
			"Child (<14)"=>$_POST['kid']);
		...
		if ($departure_id != 0) {
			...
			$query = "SELECT * FROM $db->ticket
					  WHERE ticket_del ='0' and ticket_tour_id = $tour_id";				
			$_ticket = $db->select_field_keyval($db->ticket, $query, "ticket_type", "ticket_seats");
			$_total_seats = 0;
          	   
			foreach($q[$departure_id] as $key => $value){
				...
				$order_total = 0;
				if ($key == '1')
					$order_total1 = ($adult_price * $value);
				if ($key == '2')
					$order_total2 = ($child_price * $value);
			...
			//echo $sum; echo "<hr>"; echo $departure['boat_passengers']; 
			if($_POST['ddate']!='')
			{
			 	$add_in_date=$_POST['ddate'];
			}
			else
			{
...
//			print_r($fields); die();
            ...
            $fields = array_merge($fields,
					  array("order_date"=>$add_in_date, 
							...
							"order_first_name"=>$name_arr[0],
							"order_last_name"=>$name_arr[1],
							"order_phone"=>$phone,
							"order_email" => $email,
							"order_payd" => 1,
							"order_time" => time(),
			...
			generate_order($fields, "bale boooking");
			$_SESSION['Name'] 	= ""; 
			$_SESSION['Phone'] 	= ""; 
			$_SESSION['Email'] 	= ""; 
			$_SESSION['Adult'] 	= "";
			...
			mysql_close();
		   	echo '<script>
			  location.href="booking.php?vdate='.$add_in_date.'&type=Show_Popup";			
			  </script>';
			//$status = "Order Added";
		} else {
		   echo '<script>
			  location.href="booking.php?vdate='.$add_in_date.'&expired=true";			
			  </script>';
			//header("Location:booking.php?vdate=$add_in_date&type=Show_Popup&expired=true");
			exit();	
			//$error = "The ticket(s) you have booked have just been purchased by another person.\\nPlease try again with another option.";	
		}
		$history_id = $departure_id;
