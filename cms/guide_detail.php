...
if($_REQUEST['command']=="add_guide")
{
   $tid   		= $_REQUEST['tourid'];
   $depid 		= $_REQUEST['depid'];
   $guideid   	= $_REQUEST['guide_id'];
   ...
   if($num_check > 0)
   {
       $sql_update  = "UPDATE `skipper_guide_entry` SET `sk_tour_id` = '$tid' , 
					   `sk_depid` = '$depid' , `guide_id` = '$guideid' WHERE sgid = '$sg_id'"; 
...
<table width="40%" border="0" cellpadding="5px" cellspacing="0" style="padding-left:20px;">
  <tr>
    <td height="29" align="left" valign="top" class="text_head">Guides</td>
  </tr>
  <?php
     for($i = 0; $i < $count ; $i++)
	  {
  ?>
  <tr>
  <td align="left" valign="top"><a href="guide_detail.php?tourid=<?=$tour_id?>&depid=<?=$dep_id?>&guide_id=<?=$guides[$i]['guide_id']?>&curent_date=<?=$_REQUEST['cur_date']?>&command=add_guide"><?=$guides[$i]['guide_name']?></a></td>
  </tr>
  <?php
     }
  ?>
