...
switch (@$_GET['subpage']) {
    ...
    case "voucher_step4":
        ...
        if (empty($_GET['crypt'])) {
            header("Location: booking.php?subpage=voucher_step1&error=expired");
            exit();
        }
    ...
    case "voucher_step3":
        ...
        if (sizeof($voucher_details) > 0) {
            ...
            header("Location: booking.php?subpage=voucher_step1&error=nodata");
            exit();
        }
        ...
        $smarty->assign("total", $voucher_order['voucher_order_discounted_total']);
...
*/
            $fields = array(
                ...
                "voucher_order_lastname" => $_POST['voucher_order_lastname'],
                "voucher_order_address1" => $_POST['voucher_order_address1'],
                "voucher_order_address2" => $_POST['voucher_order_address2'],
    ...
    case "voucher_details":
        ...
        if (isset($_GET['voucher_id']) && is_numeric($_GET['voucher_id'])) {
            ...
            if (empty($voucher[voucher_tour_id])) {
                header("Location: booking.php?subpage=voucher_step1&error=nodata");
            };
        } else {
            header("Location: booking.php?subpage=voucher_step1&error=nodata");
    ...
    case "package_details":
        ...
        if (!empty($package_details['package_image1'])) {
            $image1 = explode(".", $package_details['package_image1']);
            unset ($image1[sizeof($image1) - 1]);
            $smarty->assign("image1", implode(".", $image1));
        }
        if (!empty($package_details['package_image2'])) {
            $image2 = explode(".", $package_details['package_image2']);
            unset ($image2[sizeof($image2) - 1]);
            $smarty->assign("image2", implode(".", $image2));
        }
        if (!empty($package_details['package_image3'])) {
            $image3 = explode(".", $package_details['package_image3']);
            unset ($image3[sizeof($image3) - 1]);
            $smarty->assign("image3", implode(".", $image3));
        }
        if (!empty($package_details['package_image4'])) {
            $image4 = explode(".", $package_details['package_image4']);
            unset ($image4[sizeof($image4) - 1]);
            $smarty->assign("image4", implode(".", $image4));
        }
